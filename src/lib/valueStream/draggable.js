 ...
 */
function createProcessDragBehavior(d3, onDragEnd, blockHeight = 80, blockWidth = 120, connections = [], processes = []) {
  ...
  function updateConnectionsDuringDrag(processId, newX, newY) {
    ...
    connections.forEach(connection => {
      ...
      d3.select(`.connection[data-id="${connection.id}"] path`)
  ...
  return d3.drag()
    ...
    .on('drag', function(event, d) {
      ...
      d3.select(`.metrics[data-process-id="${processId}"]`)
        .attr('transform', `translate(${newX}, ${newY + metricsOffset})`);
